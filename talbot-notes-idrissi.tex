...
\section{The embedding calculus}
 ...
 cubical posets. We point the reader to~\cite{Munson2010}
 ...
 \[ F(M) \longrightarrow \operatorname{holim}_k F_k(M)\]
  ...
  point components. Moreover, for each $k\in \NN$
  the homotopy type of the space $F_k(M)$ can be 
  computed using a truncated mapping space where
  we only allow configurations of at most $k$
  points. \qed
   \end{theorem}
...
%  Span of maybe two decades. 
  ...
  \emph{configurations spaces}~\cite{knudsen2018configuration}. For each $r\in \NN$,
  ...
  braid groups by  Hurwitz~\cite{Hurwitz1891}, 
...
\[ D_m\circ D_m \longrightarrow D_m. \]
 ...
 $D_m$-module, which we will do in what follows. For
 unframed manifolds ---those with possibly non-trivial 
 tangent bundle--- one needs to look instead at \emph{framed}
 configurations, where each point (or little disk) is
 decorated with a frame\footnote{That is, we pick an element of $\mathsf{SO}(n)$ and a configuration of disks to define an element in $D_n^{\mathrm{fr}}$}, and we will use the superscript
 $\mathrm{fr}$ to denote this. 
 
 We now observe that an embedding $f:M\longrightarrow N$
 produces for us a map $D_M^\mathrm{fr} 
 \longrightarrow D_N^\mathrm{fr}$ that is
 not just a map of symmetric sequences, but in fact a map
 right $D_m^\mathrm{fr}$-modules. 
 In this way, it makes sense to form
 the mapping space 
 $\Map_{D_m^\mathrm{fr}}(D_M^\mathrm{fr},D_N^\mathrm{fr})$
 and we obtain a map
 \[ \Emb(M,N) \longrightarrow  \Map_{D_m^{\mathrm{fr}}}(D_M^\mathrm{fr},D_N^\mathrm{fr})\] 
 ...
 equivalence
 \[ \label{eq:derivedMap}
 	\Emb(M,N) \longrightarrow 
 		\mathbb R \Map_{D_m^\mathrm{fr}}
 			(D_M^\mathrm{fr},D_N^\mathrm{fr})\]
 where the right hand side is the \emph{derived}\footnote{For 
 details, the reader can consult the work of B. Fresse~\cite{Fresse2009,FresseBook1,FresseBook2}.} 
 mapping space of $D_m^\mathrm{fr}$-module maps $D_M^\mathrm{fr}\longrightarrow D_N^\mathrm{fr}$.
 \qed
  \end{theorem}
 ...
 but Longoni--Salvatore~\cite{Longoni2005}
 ...
 of Beck, Boardman-Vogt, May, Segal and Stasheff;
 see~\cite{Boardman1973,Stasheff1963,May1972}. The
 following 
 theorem, due to J. P. May, provides a useful 
 recognition principle to determine when a 
 space can be delooped.
 ...
 theorem of Kontsevich~\cite{Kontsevich2003}. 
...
is formal, this was done by Giansiracusa--Salvatore~\cite{giansiracusa2010formality}.
Later, S. Moriya~\cite{Moriya2017} and Khoroshkin--Willwacher~\cite{khoroshkin2017real} showed that
...
Lie algebras~\cite{Kohno1985,FresseBook1}
...
\emph{cyclic}, in the sense that the $S_r$ action on
 ...
 \begin{theorem}[Campos--Idrissi--Willwacher] The
...
with the two additional sets of relations:
 ...
 \begin{theorem}[Idrissi~\cite{Idrissi2018}]
 ...
 $k\in [r]$ we have that 
 $\alpha_{i,k}\beta_{i,k} = \alpha_{j,k}\beta_{j,k}$, while $\alpha_{i,k}\beta_{j,k'}=0$
 for $k=k'$ but $i\neq j$, and for $i=j$
 but $k\neq k'$.
 \end{enumerate}
 ...
 \begin{theorem}[Campos--Idrissi--Willwacher~\cite{campos2020configuration}] 
...
The note-taker would like to thank the organizers of \emph{Viva
Talbot 2021} for their assistance and flexibility during the
production of these notes, and to Najib Idrissi for pointing
out useful references and giving feedback on preliminary versions
of them. All inaccuracies, errors and omissions are my own.
Any comment, correction or feedback is very much
appreciated.
