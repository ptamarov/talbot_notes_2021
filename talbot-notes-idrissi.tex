...
\[ D_m\circ D_m \longrightarrow D_m. \]
 ...
 \begin{theorem}[Sullivan] There is a Quillen adjunction\footnote{A special kind of adjunction, see~\cite{} for details.}
 ...
 forms, but instead consists of 'piecewise linear polynomial forms'
 ...
 induces the antipodal map on $S^{m-1}$, creating a $(-1)^{m-1}$
 sign for this cohomology class. Coming from the top cohomology
 class of the sphere, $\omega_{ij}^2=0$ and, by configurations
 ...
 \item For all distinct $i,j$ we have that $\omega_{ij} = (-1)^{m-1} \omega_{ji}$.
 ...
 $D_m$ abound, see~\cite{Fresse, Petersen, Tamarkin, Horel}. One of 
 ...
 Tamarkin in~\cite{Tamarkin2003}) and the deformation quantization
 theorem of Kontsevich. For us, formality has the consequence of 
...
Lambrechts and Voli\'c~\cite{book}.

It is worth pointing out that one can show the
framed little disks operad $D_2^{\mathrm{fr}}$
is formal, this was done by Giansiracusa--Salvatore.
Later, S. Moriya and Turchin--Willwacher showed that
...
with the same element in $\sigma'$, as in the figure
...
\emph{Detour:} It is well known that each topological
